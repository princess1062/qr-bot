...
if not data:
  await update.message.reply.text("X tak jumpa QR") return

for d in data:
  await update.message.reply_text(f" MATCH TNG:\n{d}")
else:
  await update.message.reply_text(f" NOT MATCH:\n{d}")

def main():
  app : Application.builder().token("8740908330:AAGh5BymLksOzk999U_tsja6lVp3KsGQ1g").build()
